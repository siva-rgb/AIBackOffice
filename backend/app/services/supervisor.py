...
def gather_state(user_id: str) -> dict:
    ...
    overdue = [
        i for i in invoices
        if i.status == "overdue" or (i.status in ("sent", "viewed") and _days_overdue(i.due_date) > 0)
    ]
    uncategorized = sum(
        1 for t in txns
        if (not t.ai_categorized)
        or (t.ai_confidence is not None and t.ai_confidence < 0.7)
        or (t.category in (None, "other_income", "other_expense"))
    )
    return {
        "user": user,
        "profile": profile,
...
def _gather_meeting_context(user_id: str) -> dict:
    ...
    try:
        ...
        ctx["todays_client_meetings"] = [
            e.get("title") for e in todays if e.get("is_client_meeting")
        ][:5]
    ...
    try:
        if settings.SUPABASE_URL:
            ...
            items = db.table("meeting_action_items").select("due_date").eq(
                "user_id", user_id).eq("status", "open").execute().data or []
            ctx["open_action_items"] = len(items)
            ctx["overdue_action_items"] = sum(
                1 for a in items if a.get("due_date") and a["due_date"] < today_iso
            )
    except Exception:
        pass
    return ctx
...
def assess(state: dict) -> list[dict]:
    ...
    for inv in state["overdue"]:
        ...
        if days >= _WRITEOFF_DAYS and inv.follow_up_count >= 3:
            proposals.append({
                "kind": "writeoff_invoice", "severity": "warning",
                "title": f"Write off — {inv.invoice_number} ({inv.client_name})",
                "rationale": (f"{inv.invoice_number} for {cur} {inv.total:,.2f} is {days} days overdue after "
        ...
        if days >= _DEMAND_DAYS and inv.follow_up_count >= _DEMAND_MIN_FOLLOWUPS:
            proposals.append({
                "kind": "send_demand", "severity": "critical",
                "title": f"Send payment demand — {inv.invoice_number} ({inv.client_name})",
                "rationale": (f"{inv.invoice_number} for {cur} {inv.total:,.2f} is {days} days overdue "
        ...
        if attempt:
            ...
            proposals.append({
                "kind": "send_followup", "severity": "warning",
                "title": f"Send {label} — {inv.invoice_number} ({inv.client_name})",
                "rationale": (f"{inv.invoice_number} for {cur} {inv.total:,.2f} is {days} days overdue; "
                              f"a {label} is due (attempt {attempt} of 3)."),
...
def _advisories(state: dict) -> list[dict]:
    ...
    if state["cash_danger_days"] is not None:
        adv.append({"kind": "cash_danger", "severity": "critical",
    ...
    if n:
        adv.append({"kind": "unsigned_contracts", "severity": "info",
    ...
    if uc:
        adv.append({"kind": "uncategorized_txns", "severity": "info",
    ...
    if cc.get("at_risk_clients"):
        ...
        adv.append({"kind": "at_risk_clients", "severity": "warning",
    ...
    if cc.get("silent_clients"):
        adv.append({"kind": "silent_clients", "severity": "info",
    ...
    if mc.get("todays_client_meetings"):
        ...
        adv.append({"kind": "todays_meetings", "severity": "info",
    ...
    if mc.get("overdue_action_items"):
        adv.append({"kind": "overdue_action_items", "severity": "warning",
    ...
    if mc.get("unlogged_meetings_count"):
        adv.append({"kind": "unlogged_meetings", "severity": "info",
...
def run_supervisor(user_id: str, triggered_by: str = "user") -> dict:
    ...
    for p in proposals:
        ...
        store.insert_manager_task(ManagerTask(
            id=store.uid("task"), user_id=user_id, kind=p["kind"], title=p["title"],
            rationale=p["rationale"], severity=p["severity"], status="proposed",
            payload=p["payload"], source_record_type=p["source_record_type"],
            source_record_id=p["source_record_id"], created_at=_now(),
        ))
        new_tasks += 1
    ...
    if state["cash_danger_days"] is not None and not store.alert_fired_recently(user_id, "cashflow_danger", within_days=3):
        ...
        try:
            store.insert_alert(Alert(
                id=store.uid("alert"), user_id=user_id, type="cashflow_danger", severity="critical",
                title="Cash flow projected negative",
                body=_body,
                action_label="View forecast", action_url="/cashflow", read=False, created_at=_now()))
        except Exception:
            pass
        # Email the owner (fresh alert only — the dedup guard above ensures this).
        try:
            from . import owner_notify
            owner_notify.notify_critical_alert(user_id, title="Cash flow projected negative",
                                               body=_body, action_url="/cashflow")
        except Exception:
            pass
    ...
    try:
        ...
        for inv in paid_invoices:
            ...
            try:
                ...
                payment_patterns[cid] = {"avgDaysToPayment": avg, "sampleSize": n,
    ...
    payload = {
        ...
        "escalation_context": [
            {"invoiceId": k, **v}
            for k, v in list(escalation_state.items())[-5:]
            if v.get("lastAction")
        ],
        "slow_payers": [
        ...
        "top_overdue": [
            {"invoiceNumber": i.invoice_number, "client": i.client_name,
             "total": i.total, "daysOverdue": max(0, _days_overdue(i.due_date))}
    ...
    try:
        ...
        briefing = {"status_line": "Briefing unavailable — rule-based tasks still queued below.",
    ...
    agent_logger.log_action(
        ...
        input={"triggeredBy": triggered_by, "monthIncome": state["month_income"],
               "monthlyGoal": state["monthly_goal"]},
        output={"autoActions": auto_actions, "newTasks": new_tasks, "pending": len(pending),
                "statusLine": briefing.get("status_line")},
        model_used=model_used or "deterministic", tokens_used=tokens, latency_ms=latency, cost_usd=cost,
        triggered_by=triggered_by, source_record_type="supervisor",
    )
    ...
    try:
        store.set_manager_memory(user_id, {
    ...
    try:
        if task.kind == "send_followup":
            ...
            agent_logger.log_action(
                ...
                output={"status": "cancelled"}, triggered_by="user",
                source_record_type="invoice", source_record_id=inv.id)
            ...
            agent_logger.log_action(
                ...
                input={"proposalId": prop_id}, output={"status": "sent"}, triggered_by="user",
                source_record_type="proposal", source_record_id=prop_id)
            ...
            user = store.get_user(user_id)
            review = review_contract(user, text=text, source="drive",
                                     title=payload.get("fileName") or "Drive document")
            risk = getattr(review, "overall_risk", None) or (review.get("overallRisk") if isinstance(review, dict) else None)
            result = {"note": f"Reviewed '{payload.get('fileName', 'document')}' — {risk or 'done'} risk.",
...
def chat(user_id: str, message: str, history: list[dict] | None = None) -> dict:
    ...
    agent_logger.log_action(
        ...
        model_used=model_used or "deterministic", tokens_used=tokens, latency_ms=latency, cost_usd=cost,
        triggered_by="user", source_record_type="supervisor",
    )
...
def _tool_financial_summary(user_id: str, args: dict) -> dict:
    ...
    return {"currency": s["currency"], "monthIncome": s["month_income"], "monthlyGoal": s["monthly_goal"],
            "financialGoals": s["financial_goals"], "overdueCount": len(s["overdue"]),
            "overdueTotal": s["overdue_total"], "openInvoices": len(s["open_invoices"]),
            "cashDangerDays": s["cash_danger_days"], "currentBalance": s["current_balance"]}


def _tool_list_overdue(user_id: str, args: dict) -> dict:
    s = gather_state(user_id)
    return {"overdue": [
        {"number": i.invoice_number, "client": i.client_name, "total": i.total,
         "daysOverdue": max(0, _days_overdue(i.due_date)), "followUps": i.follow_up_count,
...
def _tool_list_contracts(user_id: str, args: dict) -> dict:
    return {"contracts": [
        {"title": c.title, "type": c.type, "status": c.status, "client": c.client_name}
        for c in store.list_contracts(user_id)]}


def _tool_cashflow(user_id: str, args: dict) -> dict:
    s = gather_state(user_id)
    return {"cashDangerDays": s["cash_danger_days"], "currentBalance": s["current_balance"],
            "currency": s["currency"]}
...
def _tool_list_clients(user_id: str, args: dict) -> dict:
    ...
    return {
        ...
        "clients": [
            {"name": c.name, "status": c.status, "health": c.health_score, "email": c.email}
            for c in clients
        ],
    }


_TASK_VERBS = {"send_followup": "Send follow-up", "send_demand": "Send payment demand",
               "writeoff_invoice": "Write off"}
...
def _queue_invoice_task(user_id: str, number: str, kind: str, severity: str) -> dict:
    ...
    try:
        store.insert_manager_task(ManagerTask(
            id=store.uid("task"), user_id=user_id, kind=kind,
            title=f"{verb} — {inv.invoice_number} ({inv.client_name})",
            rationale="Proposed by the manager chat at your request.", severity=severity,
            status="proposed", payload={"invoiceId": inv.id, "invoiceNumber": inv.invoice_number},
            source_record_type="invoice", source_record_id=inv.id, created_at=_now()))
...
def _tool_run_review(user_id: str, args: dict) -> dict:
    ...
    return {"autoActions": r["autoActions"], "pendingDecisions": len(r["pendingTasks"]),
            "statusLine": r["briefing"]["statusLine"]}
...
def _tool_recall_memory(user_id: str, args: dict) -> dict:
    ...
    return {"memories": [
        {"content": h["content"], "kind": h.get("kind"), "source": h.get("source"),
         "score": h.get("_score")}
        for h in hits
...
def _tool_list_tasks(user_id: str, args: dict) -> dict:
    ...
    if client_name:
        ...
        match = next((c for c in store.list_clients(user_id)
                      if (c.name or "").lower() == target), None)
        if not match:
    ...
    tasks = store.list_tasks(
        ...
        status=status, statuses=None if status else OPEN_STATUSES,
    )
    return {
...
def _tool_propose_task(user_id: str, args: dict) -> dict:
    ...
    t = create_task(user_id, {
        "title": title, "client_id": client_id, "due_date": args.get("due_date"),
        "priority": args.get("priority") or "medium", "owner": "me", "source": "agent",
    })
...
_TOOLS = [
    ...
    {"type": "function", "function": {"name": "list_overdue_invoices",
        "description": "Overdue invoices with amounts, days overdue, prior follow-ups, contract link.",
        "parameters": {"type": "object", "properties": {}, "required": []}}},
    {"type": "function", "function": {"name": "list_contracts",
        "description": "The owner's contracts and their status.",
        "parameters": {"type": "object", "properties": {}, "required": []}}},
    {"type": "function", "function": {"name": "get_cashflow",
        "description": "Cash-flow danger window and current balance.",
        "parameters": {"type": "object", "properties": {}, "required": []}}},
    {"type": "function", "function": {"name": "list_clients",
        "description": ("The owner's clients — total count, plus each client's name, status "
                        "(active/prospect/inactive), health score, and email. Use this for ANY question "
                        "about clients or how many clients the owner has."),
        "parameters": {"type": "object", "properties": {}, "required": []}}},
    {"type": "function", "function": {"name": "propose_follow_up",
        "description": ("Queue a follow-up email to a client for an overdue invoice FOR THE OWNER'S "
                        "APPROVAL. Does NOT send it. Provide the invoice number."),
        "parameters": {"type": "object", "properties": {"invoice_number": {"type": "string"}},
                       "required": ["invoice_number"]}}},
    {"type": "function", "function": {"name": "propose_payment_demand",
        "description": ("Queue a formal payment-demand letter for a seriously overdue invoice FOR THE "
                        "OWNER'S APPROVAL. Does NOT send it. Provide the invoice number."),
        "parameters": {"type": "object", "properties": {"invoice_number": {"type": "string"}},
                       "required": ["invoice_number"]}}},
    {"type": "function", "function": {"name": "propose_write_off",
        "description": ("Queue writing off a very stale, uncollectable invoice as bad debt FOR THE "
                        "OWNER'S APPROVAL (does not cancel it directly). Provide the invoice number."),
        "parameters": {"type": "object", "properties": {"invoice_number": {"type": "string"}},
                       "required": ["invoice_number"]}}},
    {"type": "function", "function": {"name": "run_full_review",
    ...
    {"type": "function", "function": {"name": "query_graph",
        "description": ("Traverse the relationship memory for ONE client — everything linked to them: "
    ...
    {"type": "function", "function": {"name": "recall_memory",
        "description": ("Semantic recall over your durable memory of PAST context — learned facts, "
    ...
    {"type": "function", "function": {"name": "list_tasks",
        "description": ("Open client WORK from the task ledger — what's outstanding, overdue or "
        ...
        "parameters": {"type": "object", "properties": {
            "client_name": {"type": "string"}, "status": {"type": "string"}}, "required": []}}},
    {"type": "function", "function": {"name": "propose_task",
        "description": ("Add a task to the owner's ledger so a piece of work is tracked and can't be "
        ...
        "parameters": {"type": "object", "properties": {
            "title": {"type": "string"}, "client_name": {"type": "string"},
...
def chat_agentic(user_id: str, message: str, history: list[dict] | None = None) -> dict:
    ...
    agent_logger.log_action(
        ...
        model_used=model or settings.MODEL_NAME, tokens_used=in_tok + out_tok,
