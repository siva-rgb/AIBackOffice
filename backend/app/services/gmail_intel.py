...
_PUBLIC_EMAIL_DOMAINS = {
    ...
    "gmx.com", "gmx.net", "zoho.com", "mail.com", "yandex.com", "fastmail.com",
}
...
def sync_client_email_intel(user_id: str, force_refresh: bool = False) -> None:
    ...
    clients = db.table("clients").select("*").eq(
        "user_id", user_id).eq("status", "active").not_.is_("email", "null").execute().data
...
def _process_client_threads(
    ...
    if not force_refresh:
        ...
        if cache and cache[0].get("processed_at"):
            processed = datetime.fromisoformat(
                cache[0]["processed_at"].replace("Z", "+00:00")
            )
    ...
    try:
        ...
        if summary:
            remember(user_id, "email_intel", f"{client_name}: {summary}",
                     client_id=client_id, ref_type="email_intel_cache",
                     ref_id=str(client_id), salience=0.6, source="email")
    except Exception:
        pass
...
def _fetch_client_threads(service, query: str) -> list:
    result = service.users().threads().list(
        userId="me", q=query, maxResults=_MAX_THREADS_FETCH,
    ).execute()
...
def _analyze_threads(
    user_id: str, client_name: str, client_email: str,
    addresses: list[str], threads: list, service,
) -> dict:
    ...
    try:
        ...
        last_contact_direction = (
            "from_client" if any(a in from_email for a in addresses) else "from_me"
        )
    except Exception:
        pass
...
Be conservative. If the threads are too brief, return neutral with no action."""
    ...
    try:
        ai = get_ai()
        call = generate_with_retry(lambda: ai.analyze_email_threads({
            "prompt": prompt,
            "client_name": client_name,
        }))
    ...
    agent_logger.log_action(
        ...
        input={"client_email": client_email,
               "addresses_matched": len(addresses),
               "threads_processed": len(thread_summaries)},
        output={"sentiment": intel.get("sentiment"),
    ...
    if settings.SUPABASE_URL:
        ...
        if expiration:
            exp_iso = datetime.fromtimestamp(
                int(expiration) / 1000, tz=timezone.utc).isoformat()
        try:
...
def handle_push_notification(email_address: str) -> None:
    ...
    if not (settings.SUPABASE_URL and email_address):
        return
    rows = _db().table("google_connections").select("user_id").eq(
        "google_email", email_address).eq("connected", True).execute().data
    if not rows:
        return
    try:
