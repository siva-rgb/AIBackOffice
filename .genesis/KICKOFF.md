# KICKOFF — paste this to start or resume a KORA session cold
...
```
PROJECT: KORA — AI-native back-office SaaS (Next.js 14 frontend + FastAPI backend,
Supabase, Stripe, Google/Notion connectors, OpenAI-compatible LLM gateway).
...
- milestone-specific: security-engineering (M1) · llmops-ai-agents (M2, M4) ·
  distributed-systems (M3)

  ⚠ MACHINE REALITY (verified 2026-07-23 — do not waste turns on this):
...
- A real backend/.env exists → the app defaults to KORA_DATA_BACKEND=supabase.
  Tests and demos MUST pass KORA_DATA_BACKEND=mock explicitly.
...
Then:
1. Pick the next unstarted milestone (or resume from CURRENT.md). Next up: M1 — plan gating.
