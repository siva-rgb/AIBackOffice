...
def get_todays_meetings_with_clients(user_id: str) -> list[dict]:
    ...
    clients = db.table("clients").select("id, name, email").eq("user_id", user_id).execute().data
    email_map = {c["email"].lower(): c for c in clients if c.get("email")}

    enriched = []
    for ev in events:
        attendees = ev.get("attendees", [])
        matched = [email_map[a.get("email", "").lower()]["name"] for a in attendees if a.get("email", "").lower() in email_map]
        start = ev.get("start", {})
        enriched.append(
            {
                "id": ev["id"],
...
def get_unlogged_past_meetings(user_id: str) -> list[dict]:
    ...
    existing = db.table("meetings").select("google_event_id").eq("user_id", user_id).not_.is_("google_event_id", "null").execute().data
    ...
    for ev in events:
        if ev["id"] in logged_ids:
            continue
        matched = [email_map[a.get("email", "").lower()] for a in ev.get("attendees", []) if a.get("email", "").lower() in email_map]
        if matched:
            unlogged.append(
                {
                    "event_id": ev["id"],
...
def find_availability_slots(
    ...
    while len(suggestions) < 3 and current < now + timedelta(days=days_ahead):
        if current.weekday() < 5 and current.hour < 17:
            ...
            overlaps = any(
                datetime.fromisoformat(b["start"].replace("Z", "+00:00")) < slot_end and datetime.fromisoformat(b["end"].replace("Z", "+00:00")) > current
                for b in busy
            )
            if not overlaps:
                suggestions.append(
